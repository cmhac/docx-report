"""generates docx files with custom reports for each station"""
...
class DocxReport:
    """generates a docx file with custom reports for each station"""

    def __init__(self, local_station_name: str) -> None:
        self.doc = docx.Document()
        self.local_station_name = local_station_name
        self._add_heading()
    ...
    def _add_heading(self) -> None:
        ...
        subtitle_p.add_run(
            f"Generated by Chris Hacker on {datetime.now().strftime('%B %-d, %-I:%-M %p')}"
        )
        subtitle_p.style = "Subtitle"
        # add the title
        self.doc.add_heading(
            f"{self.local_station_name} custom car theft data report", 0
        )
